...
ENTITY_PROCESSING_TIME = Histogram(
    'content_enrichment_entity_processing_seconds',
    'Time spent processing entities'
)
FACT_VERIFICATION_TIME = Histogram(
    'content_enrichment_fact_verification_seconds',
    'Time spent verifying facts'
)
ENTITY_COUNT = Counter(
    'content_enrichment_entity_total',
    'Total number of entities processed'
)
FACT_CHECK_COUNT = Counter(
    'content_enrichment_fact_checks_total',
    'Total number of fact checks performed'
)
            ...
            try:
                ...
                if ent.label_ in context_terms:
                    for context in context_terms[ent.label_]:
                        ...
                        if wiki_results:
                            self.logger.info(f"Found Wikipedia results for '{search_term} {context}': {wiki_results}")
                            break
                else:
