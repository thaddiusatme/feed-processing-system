...
topic_processing_time = Histogram(
    "content_topic_processing_seconds", "Time spent processing topic analysis", ["operation"]
)
topic_errors = Counter(
    "content_topic_errors_total", "Number of errors in topic analysis", ["error_type"]
)
...
class TopicAnalyzer:
    ...
    def extract_topics(
        ...
        try:
            ...
            # Cluster documents
            clusters = DBSCAN(eps=max_distance, min_samples=min_cluster_size, metric="cosine").fit(
                vectors
            )
            ...
            for cluster_id in set(clusters.labels_):
                ...
                cluster_docs = [
                    doc_texts[i] for i, label in enumerate(clusters.labels_) if label == cluster_id
                ]
                cluster_ids = [
                    doc_ids[i] for i, label in enumerate(clusters.labels_) if label == cluster_id
                ]
    ...
    def _update_trend_data(self, current_topics: List[Topic], documents: List[Dict[str, str]]):
        ...
        for topic in current_topics:
            ...
            trend.volume_trend = [(t, v) for t, v in trend.volume_trend if t >= cutoff]

            # Update related topics
            related = self._find_related_topics(topic.id, current_topics)
            trend.related_topics = related

    def _find_related_topics(self, topic_id: int, topics: List[Topic]) -> List[Tuple[int, float]]:
        ...
        for other in topics:
            ...
            overlap = len(set(target.keywords).intersection(other.keywords)) / len(
                set(target.keywords).union(other.keywords)
            )
    ...
    def _identify_trending_topics(self) -> List[int]:
        ...
        for topic_id, trend in self.trend_data.items():
            ...
            is_trending = all(volumes[i] <= volumes[i + 1] for i in range(len(volumes) - 1))
