...
class PriorityQueue:
    ...
    def enqueue(self, item: QueueItem, priority: Priority = Priority.NORMAL) -> bool:
        ...
        with self._lock:
            if self._size >= self._max_size:
                if item.priority == Priority.HIGH:
                    ...
                    if len(self._queues[Priority.LOW]) > 0:
                        ...
                        self.metrics.increment("queue_overflows", labels={"priority": item.priority.name.lower()})
                        return False
                else:
                    self.metrics.increment("queue_overflows", labels={"priority": item.priority.name.lower()})
                    return False
            ...
            self.metrics.increment("enqueued_items", labels={"priority": item.priority.name.lower()})
            self._size += 1
