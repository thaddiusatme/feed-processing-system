...
class ContentQueue:
    ...
    def _clean_old_hashes(self) -> None:
        ...
        self.processed_ids = {h for h in self.processed_ids if h > cutoff_time}
    ...
    def dequeue(self) -> Optional[QueueItem]:
        ...
        with self.lock:
            ...
            now = datetime.now()
            ready_items = [
                item for item in self.items if not item.next_retry or item.next_retry <= now
            ]
    ...
    def get_queue_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "oldest_item_age": (
                (datetime.now() - self.items[0].timestamp).total_seconds() if self.items else 0
            ),
        }
