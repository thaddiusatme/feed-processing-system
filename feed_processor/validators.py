...
class FeedValidator:
    ...
    def _validate_content_type(content_type: str, errors: List[str]) -> None:
        ...
        if content_type and content_type not in FeedValidator.CONTENT_TYPES:
            errors.append(f"Invalid content type. Must be one of: {', '.join(FeedValidator.CONTENT_TYPES)}")
    ...
    def _validate_priority(priority: str, errors: List[str]) -> None:
        ...
        if priority and priority not in FeedValidator.PRIORITY_LEVELS:
            errors.append(f"Invalid priority. Must be one of: {', '.join(FeedValidator.PRIORITY_LEVELS)}")
        ...
        if publish_date:
            try:
                if isinstance(publish_date, str):
                    ...
                    normalized['metadata']['source']['publishDate'] = datetime(*publish_date[:6]).isoformat()
