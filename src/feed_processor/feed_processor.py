...
import requests
import structlog
from .priority_queue import PriorityQueue, Priority, QueueItem
...
class FeedProcessor:
    ...
    def _determine_priority(self, item: Dict[str, Any]) -> Priority:
        ...
        elif datetime.fromisoformat(item["published"]) > datetime.now(timezone.utc).replace(hour=0, minute=0):
        ...
        try:
            ...
            if items:
                ...
                success = self.webhook_manager.send_webhook(
                    payload=[item.content for item in items],
                    retries=3
                )
                ...
                if success:
                    self.metrics.increment("items_processed", len(items), labels={"status": "success"})
