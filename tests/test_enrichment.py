...
from feed_processor.content_analysis.enrichment import (
    ContentEnricher,
    Entity,
    FactCheck
)
...
def test_entity_identification(enricher, sample_text):
    ...
    assert any(text in ['apple', 'apple inc.', 'apple inc'] for text, label in entity_labels if label == 'ORG')
    
    # Check for person entities
    assert any(text in ['tim cook', 'tim', 'cook'] for text, label in entity_labels if label == 'PERSON')
