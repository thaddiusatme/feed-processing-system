...
def run_locust(scenario: str, duration: str, host: str) -> None:
    ...
    cmd = [
        ...
        "-f", "locustfile.py",
        "--headless",
        "-u", get_scenario_config(scenario)["users"],
        "-r", get_scenario_config(scenario)["spawn_rate"],
        "--run-time", duration,
        "--host", host
    ]
...
def main():
    ...
    parser.add_argument(
        "--duration",
        default="5m",
        help="Duration of the test (e.g., '1h', '30m', '5m')"
    )
    parser.add_argument(
        "--recovery-type",
    ...
    parser.add_argument(
        ...
        help="Host URL of the feed processing system"
    )
